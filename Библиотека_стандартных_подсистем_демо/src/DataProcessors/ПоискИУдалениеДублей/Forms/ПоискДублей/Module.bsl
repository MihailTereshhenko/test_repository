...
Процедура ШагМастераДалее()
	...
	ОбработчикДалееЗаверешние = Новый ОписаниеОповещения("ШагМастераДалееЗавершение", ЭтотОбъект);
	Если ТекущаяСтраница = Элементы.ШагПоискНеВыполнялся Тогда
		...
		ВыполнитьОбработкуОповещения(ОбработчикДалееЗаверешние, Шаг.Индекс + 1);
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ШагВыборОсновногоЭлемента Тогда
		...
		Если ЗначениеЗаполнено(Шаг.ПриНажатииКнопкиДалее) Тогда
			...
			Обработчик = Новый ОписаниеОповещения(Шаг.ПриНажатииКнопкиДалее, ЭтотОбъект, 
							Новый Структура("ОбработчикЗавершения", ОбработчикДалееЗаверешние));
	...
	ИначеЕсли ТекущаяСтраница = Элементы.ШагНеудачныеЗамены Тогда
		...
		ВыполнитьОбработкуОповещения(ОбработчикДалееЗаверешние, Элементы.ШагВыполнениеУдаления);
		
	ИначеЕсли ТекущаяСтраница = Элементы.ШагДублейНеНайдено Тогда
		
		ВыполнитьОбработкуОповещения(ОбработчикДалееЗаверешние, Элементы.ШагВыполнениеПоиска);
...
Процедура ШагВыборОсновногоЭлементаПриНажатииКнопкиДалее(ОписаниеСледующегоШага, ДополнительныеПараметры) Экспорт
	...
	Если ВозвращеноМеньшеЧемНайдено Тогда
		...
		ОбработчикОтвета = Новый ОписаниеОповещения("ШагВыборОсновногоЭлементаПриНажатииКнопкиДалееЗаверешние", ЭтотОбъект, ПараметрыОбработчика);
		ПоказатьВопрос(
			ОбработчикОтвета, 
...
&НаКлиенте         
Процедура ШагВыборОсновногоЭлементаПриНажатииКнопкиДалееЗаверешние(Результат, ДополнительныеПараметры) Экспорт
